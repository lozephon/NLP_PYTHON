...
def checkfile(dirname, filename):
    ...
    #print filename.decode('cp949').encode('utf-8')
    if isEncoding:
...
def displayForm(dirname, filename):
    ...
    '''
    symbols = ['‘', '’', '“', '”', "'", "%"]
    symbols = [symbol.decode('utf-8').encode('euc-kr') for symbol in symbols]
    symbolTags = ['&#39;', '&#39;', '&#34;', '&#34;', '&#39;', '&#37;']
    symbolTags = [symbolTag.decode('utf-8').encode('euc-kr') for symbolTag in symbolTags]
    symbolDic = {}
    
    for symIdx in range(len(symbols)):
        symbolDic[symbols[symIdx]] = symbolTags[symIdx]
    '''
    ...
    for line in lines:
        ...
        for word in words:
            ...
            for symbol in symbols:
                newWord = re.sub(r'%s' % symbol, symbolDic[symbol], newWord)
            tempWords.append(newWord) 
...
#        if isEncoding:
#            selectHTML = selectHTML.decode('utf-8').encode('euc-kr')
